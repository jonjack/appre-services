...
impl RuntimeConfig {
    ...
    pub fn from_env() -> Result<Self, NotificationError> {
        ...
        let app_name = std::env::var("APP_NAME")
            .map_err(|_| NotificationError::ConfigurationError("APP_NAME not set".to_string()))?;
        
        // ENVIRONMENT comes from Lambda environment variable set by CDK
        let environment = std::env::var("ENVIRONMENT")
            .map_err(|_| NotificationError::ConfigurationError("ENVIRONMENT not set".to_string()))?;
...
mod tests {
    ...
    fn test_ses_template_naming() {
        ...
        assert_eq!(config.ses_template("otp"), "appre-test-otp");
        assert_eq!(config.ses_template("welcome"), "appre-test-welcome");
        assert_eq!(config.ses_template("complete-registration-user-info"), "appre-test-complete-registration-user-info");
        assert_eq!(config.ses_template("complete-registration-stripe"), "appre-test-complete-registration-stripe");
        assert_eq!(config.ses_template("newsletter"), "appre-test-newsletter");
    }
    ...
    fn test_different_environments() {
        ...
        let prod_config = RuntimeConfig {
            app_name: "appre".to_string(),
            environment: "prod".to_string(),
        };
        
        assert_eq!(test_config.ses_template("otp"), "appre-test-otp");
        assert_eq!(prod_config.ses_template("otp"), "appre-prod-otp");
        
        assert_eq!(test_config.sqs_queue("email-queue"), "appre-test-email-queue");
        assert_eq!(prod_config.sqs_queue("email-queue"), "appre-prod-email-queue");
    }
}